...
var fetchgit = require('./sources/fetchgit.js');
...
function npmToNix(inputJSON, outputNix, compositionNix, nodeEnvNix, lockJSON, supplementJSON, supplementNix, production, includePeerDependencies, flatten, nodePackage, registries, noCopyNodeEnv, bypassCache, useFetchGitPrivate, stripOptionalDependencies, gitRepository, callback) {
    var obj;
    var lock;
    ...
    slasp.sequence([
        function(callback) {
            if(gitRepository !== null) {
                fetchgit.fetchGitRepository("package", gitRepository, function(err, repository) {
                    if(err) {
                        callback(err);
                    } else {
                        baseDir = repository.repositoryDir;
                        inputJSON = path.join(repository.repositoryDir, inputJSON);
                        callback();
                    }
                });
            } else {
        ...
        function(callback, inputData) {
            ...
            if(typeof obj == "object" && obj !== null) {
                if(Array.isArray(obj)) {
                    ...
                    // Parse package.json
                    var localVersionSpec;

                    if(gitRepository === null) {
                        localVersionSpec = "./.";
                    } else {
                        localVersionSpec = gitRepository;
                    }

                    expr = new PackageExpression(deploymentConfig, lock, baseDir, obj.name, localVersionSpec);
