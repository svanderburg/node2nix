 ...
 */
Source.constructSource = function(registries, baseDir, outputDir, dependencyName, gitRef, versionSpec, includeHash, stripOptionalDependencies) {
    ...
    if(parsedVersionSpec !== null) { // If the version is valid semver range, fetch the package from the NPM registry
        ...
        return new GitSource(baseDir, dependencyName, gitRef, GitSource.composeGitURL("git+https://github.com", parsedUrl), includeHash);
        ...
        return new GitSource(baseDir, dependencyName, gitRef, GitSource.composeGitURL("git+https://gist.github.com", parsedUrl), includeHash);
        ...
        return new GitSource(baseDir, dependencyName, gitRef, GitSource.composeGitURL("git+https://bitbucket.org", parsedUrl), includeHash);
        ...
        return new GitSource(baseDir, dependencyName, gitRef, GitSource.composeGitURL("git+https://gitlab.com", parsedUrl), includeHash);
    } else if(typeof parsedUrl.protocol == "string" && parsedUrl.protocol.substr(0, 3) == "git") { // If the version is a Git URL do a Git checkout
        return new GitSource(baseDir, dependencyName, gitRef, versionSpec, includeHash);
        ...
        return new GitSource(baseDir, dependencyName, gitRef, "git+https://github.com/"+versionSpec, includeHash);
