...
NPMRegistrySource.prototype.fetch = function(callback) {
    ...
    slasp.sequence([
        ...
        function(callback, client) {
            ...
            const hasScope = self.dependencyName.startsWith("@")
            if (hasScope) {
                const scope = self.dependencyName.slice(0, self.dependencyName.indexOf("/"));
                const found = self.registries.find(registry => registry.scope == scope)
                if (found) {
            ...
            var url = selectedRegistry.URL + "/" + self.dependencyName.replace("/", "%2F"); // Escape / to make scoped packages work
