...
LocalSource.prototype.fetch = function(callback) {
    ...
    self.config = JSON.parse(fs.readFileSync(path.join(resolvedPath, "package.json")));

    // Parse package-lock.json file (if it exists)
    var lockFile = path.join(resolvedPath, "package-lock.json");
