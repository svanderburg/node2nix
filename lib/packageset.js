 ...
 */
function PackageSet(registryURL, baseDir, outputDir) {
 ...
 *     an external source
 */
PackageSet.prototype.fetchMetaData = function(dependencyName, versionSpec, callback) {
    ...
    if(parsedVersionSpec !== null) { // If the version is valid semver range, fetch the package from the NPM registry
        fetchMetaDataFromNPMRegistry(dependencyName, parsedVersionSpec, this.registryURL, callback);
        ...
        fetchMetaDataFromLocalDirectory(this.baseDir, this.outputDir, parsedUrl.path, callback);
        ...
        fetchMetaDataFromLocalDirectory(this.baseDir, this.outputDir, versionSpec, callback);
        ...
        fetchMetaDataFromNPMRegistry(dependencyName, versionSpec, this.registryURL, callback);
    }
};

/**
 * Checks if any parent package provides a dependency corresponding to given
 * dependency name and version specification.
 *
 ...
 * @param {String} versionSpec Version specifier of a Node.js package, such as
 *     an exact version number, version range, URL, or GitHub identifier
 * @return {Boolean} true if a parent provides the requested dependency that
 *     does not conflict
 */
PackageSet.prototype.parentProvidesDependency = function(metadata, dependencyName, versionSpec) {
    if(metadata === null) { // If we have reached a leaf of the dependency graph, we know there are no dependencies
        return false;
    } else {
        ...
        if(resolvedDependency === undefined) { // If the parent does not provide a package with the dependency name, consult the parent's parent
            return this.parentProvidesDependency(metadata.parent, dependencyName, versionSpec);
            ...
            return semver.satisfies(resolvedDependency.packageObj.version, versionSpec);
        }
    }
}
 ...
 *     If the operation succeeded, an object is returned that in which each key
 *     corresponds to a package name and each value refers to a metadata object
 *     for the package. Each metadata object has a reference to the parent's
 *     metadata object
 */
PackageSet.prototype.resolveDependencies = function(metadata, dependencies, callback) {
    var self = this;
    ...
    if(typeof dependencies == "object" && dependencies !== null) { // If the dependency set is empty then we don't have to generate anything
        ...
        slasp.fromEach(function(callback) {
            ...
            if(metadata === null || !self.parentProvidesDependency(metadata.parent, dependencyName, versionSpec)) { // Only include a dependency, if none of the parents can provide the package that matches the version specifier
                slasp.sequence([
                    function(callback) {
                        // Fetch package meta data from an external source
                        self.fetchMetaData(dependencyName, versionSpec, callback);
                    },
                    ...
                    function(callback, dependencyMetadata) {
                        ...
                        self.sources[dependencyMetadata.identifier] = dependencyMetadata;
                        callback();
                    }
                ], callback);
            } else {
 ...
 *     If an error occured, the error parameter is set to contain an error message.
 */
PackageSet.prototype.resolveAllDependencies = function(metadata, production, includePeerDependencies, callback) {
    var self = this;
    var resolvedDependencies;
    
    slasp.sequence([
        function(callback) {
            self.resolveDependencies(metadata, metadata.packageObj.dependencies, callback);
        },
        ...
        function(callback, dependencies) {
            ...
            if(production) {
                ...
                self.resolveDependencies(metadata, metadata.packageObj.devDependencies, callback);
            }
        },
        ...
        function(callback, devDependencies) {
            ...
            if(resolvedDependencies === undefined) {
                resolvedDependencies = devDependencies;
            } else {
                for(var dependencyName in devDependencies) {
                    resolvedDependencies[dependencyName] = devDependencies[dependencyName];
                }
            }
            
            if(includePeerDependencies) {
                self.resolveDependencies(metadata, metadata.packageObj.peerDependencies, callback);
            } else {
                callback();
            }
        },
        ...
        function(callback, peerDependencies) {
            /* Merge the previous dependency arguments together */
            if(resolvedDependencies === undefined) {
                resolvedDependencies = peerDependencies;
            } else {
                for(var dependencyName in peerDependencies) {
                    resolvedDependencies[dependencyName] = peerDependencies[dependencyName];
                }
            }
            
            /* Add the resolve dependencies to the metadata */
            metadata.resolvedDependencies = resolvedDependencies;
            ...
            if(resolvedDependencies === undefined) {
                ...
                slasp.fromEach(function(callback) {
                    ...
                    self.resolveAllDependencies(resolvedDependency, true /* Never include development dependencies of transitive dependencies */, includePeerDependencies, callback);
                }, callback);
            }
        }
    ], callback);
 ...
 */
PackageSet.prototype.generatePackageArgsExpr = function(metadata, production, includePeerDependencies, callback) {

    var self = this;

    slasp.sequence([
        function(callback) {
            self.resolveAllDependencies(metadata, production, includePeerDependencies, callback);
        },
