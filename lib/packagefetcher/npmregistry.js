    ...
    slasp.sequence([
        ...
        function(callback, result) {
            ...
            if(result === undefined) {
                ...
                if(semver.validRange(versionSpec) === null) { // If the version specifier is not a valid semver range, we consider it a tag which we need to resolve to a version
                    versionSpec = result['dist-tags'][versionSpec];
                }
                
                // Take the right version's metadata from the versions object
                var resolvedVersion = semver.maxSatisfying(versionIdentifiers, versionSpec);
