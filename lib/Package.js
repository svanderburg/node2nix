 ...
 */
function Package(deploymentConfig, lock, parent, name, gitRef, versionSpec, includeHash, baseDir, production, sourcesCache) {
    this.deploymentConfig = deploymentConfig;
    this.lock = lock;
    this.parent = parent;
    this.production = production;
    this.sourcesCache = sourcesCache;
    this.source = Source.constructSource(deploymentConfig.registries, baseDir, deploymentConfig.outputDir, name, gitRef, versionSpec, includeHash, deploymentConfig.stripOptionalDependencies);
    ...
    if(dependencies === undefined) {
        ...
        slasp.fromEach(function(callback) {
            ...
            if(self.isBundledDependency(dependencyName)) {
                ...
                var pkg = new Package(self.deploymentConfig, self.lock, self, dependencyName, self.gitRef, versionSpec, self.includeHash, self.source.baseDir, true /* Never include development dependencies of transitive dependencies */, self.sourcesCache);
    ...
    if(dependencyObj.dependencies === undefined) {
        ...
        slasp.fromEach(function(callback) {
            ...
            if(dependency.bundled) { // Bundled dependencies should not be included
                ...
                var pkg = new Package(self.deploymentConfig, self.lock, self, dependencyName, self.gitRef, dependency.version, self.includeHash, self.source.baseDir, self.production, self.sourcesCache);
