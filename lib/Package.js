 ...
 */
function Package(deploymentConfig, parent, name, versionSpec, baseDir, production, sourcesCache) {
    this.deploymentConfig = deploymentConfig;
    this.parent = parent;
    this.production = production;
    this.sourcesCache = sourcesCache;
    this.source = Source.constructSource(deploymentConfig.registryURL, baseDir, deploymentConfig.outputDir, name, versionSpec);
...
Package.prototype.findMatchingProvidedDependencyByParent = function(name, versionSpec) {
    if(this.parent === null) {
        ...
        if(dependency === undefined) {
            return this.parent.findMatchingProvidedDependencyByParent(name, versionSpec); // If the parent does not provide the dependency, try the parent's parents
 ...
 * @method
 * @param {String} dependencyName Name of the dependency
 * @param {Package} package Package to bundle in the node_modules/ sub folder
 */
Package.prototype.bundleDependency = function(dependencyName, package) {
    if(this.deploymentConfig.flatten) { // In flatten mode, bundle dependency with the highest parent where it is not conflicting
        if(this.parent !== null && (this.parent.providedDependencies[dependencyName] === undefined || this.parent.requiredDependencies[dependencyName] === undefined)) {
            this.parent.bundleDependency(dependencyName, package);
        } else {
            this.providedDependencies[dependencyName] = package;
        }
    } else {
        this.providedDependencies[dependencyName] = package; // Bundle the dependency with the package
    }
};
    ...
    if(dependencies === undefined) {
        ...
        slasp.fromEach(function(callback) {
            ...
            if(self.isBundledDependency(dependencyName)) {
                ...
                var package = new Package(self.deploymentConfig, self, dependencyName, versionSpec, self.source.baseDir, true /* Never include development dependencies of transitive dependencies */, self.sourcesCache);

                slasp.sequence([
                    function(callback) {
                        package.source.fetch(callback);
                    },

                    function(callback) {
                        self.sourcesCache.addSource(package.source);
                        self.requiredDependencies[dependencyName] = package;
                        self.bundleDependency(dependencyName, package);
                        resolvedDependencies[dependencyName] = package;
                        callback();
                    }
                ], callback);
            } else {
    ...
    slasp.sequence([
        ...
        function(callback) {
            ...
            slasp.fromEach(function(callback) {
                ...
                dependency.resolveDependencies(callback);
            }, callback);
        }
    ], callback);
};
