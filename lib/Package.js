...
function Package(deploymentConfig, lock, parent, name, versionSpec, baseDir, production, sourcesCache) {
    ...
    this.source = Source.constructSource(deploymentConfig.registryURL, deploymentConfig.registryAuthToken, baseDir, deploymentConfig.outputDir, name, versionSpec, deploymentConfig.stripOptionalDependencies);
...
Package.prototype.resolveDependencies = function(callback) {
    if(this.lock === undefined) { // If no lock file is present, let the tool fetch all dependencies and metadata
        if (this.source.lockFile !== undefined) {
            this.resolveDependenciesFromLockedDependencies(this.source.lockFile, callback);
        } else {
