...
function generatePackageExpr(packageSet, packageObj, baseDir, production, includePeerDependencies, flatten, callback) {
    ...
    slasp.sequence([
        function(callback) {
            /* Check if the mandatory package properties are provided */
            if(metadata.packageObj.name === undefined) {
                callback("We have encountered a package without the mandatory name attribute!");
            } else if(metadata.packageObj.version === undefined) {
                callback("Package: "+metadata.packageObj.name+" lacks the mandatory version attribute!");
            } else {
